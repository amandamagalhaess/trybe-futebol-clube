...
export default class LeaderboardModel {
  ...
  async getLeaderboardAway(): Promise<ILeaderboard[]> {
    const leaderboard = (await this.getTeams()).map(async (team) => {
      ...
      const wins = teamMatches.filter((match) => match.awayTeamGoals > match.homeTeamGoals);
      const draws = teamMatches.filter((match) => match.awayTeamGoals === match.homeTeamGoals);
      const losses = teamMatches.filter((match) => match.awayTeamGoals < match.homeTeamGoals);
      ...
      return {
        ...
        totalDraws: draws.length,
        totalLosses: losses.length,
        goalsFavor: teamMatches.reduce((total, match) => total + match.awayTeamGoals, 0),
        goalsOwn: teamMatches.reduce((total, match) => total + match.homeTeamGoals, 0),
      };
    });
